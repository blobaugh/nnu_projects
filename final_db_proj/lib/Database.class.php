	...
	*/
	 ...
	 * Example:
        ...
        public function Insert($Table, $Data) {
                 ...
                 */
                if(is_array($Data)) {
                        $query .= $this->BuildQueryFromArray($Data, $Table);
                } else {
         ...
         * @return String
         **/
        private function BuildQueryFromArray($Data, $Table = '') {
                $a = array();
                if(!is_array($Data)) return $a; // Assure data is an array

                if($Table != '') {
                        // Only add valid columns into the query
                        $columns = $this->Columns($Table);
                        foreach($columns AS $c) {
			        if(isset($Data[$c['Field']])) {
			        	$a[] = "`" . $c['Field'] . "`='{$Data[$c['Field']]}' ";
			        }
		        }
                } else {
                        // Don't bother checking the columns
                        foreach($Data as $k => $v) {
                                $a[] = "$k='$v'";
                        }
