...
require_once($Reg->Get('doc_root') . "cms/Settings.php");
...
if($route[$i] != '') { // The user is trying to view a page somewheres. Try to load it
        ...
        $controller = $Reg->Get('cms_path_controllers')  . $route[$i];
	...
	if(is_dir($controller)) {
		...
		require_once($Reg->Get('cms_path_controllers') . $Reg->Get('cms_default_controller') . ".php");
	}
